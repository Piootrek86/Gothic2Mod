...
func int Print_TextFieldPxl(var int x, var int y, var string text, var string font) {
    return Print_TextField(Print_ToVirtual(x, PS_X), Print_ToVirtual(y, PS_Y), text, font, Print_ToVirtual(Print_GetFontHeight(font), PS_Y));  // TUTAJ
};
...
func void PrintS_Ext(var string txt, var int color) {
    ...
    Anim8q(v, 60,   PF_WaitTime,    A8_Wait);
    ...
    p.tv_Text = Print_Ext(PF_PrintX+3000, PF_PrintY, txt, PF_Font, color, -1);
...
func void PrintS_Pow(var string txt, var int color) {
    var int h; h = new(gCPrintS@);
    var gCPrintS p; p = get(h);
    var int v;
